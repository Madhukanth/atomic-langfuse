...
import { Fragment, useMemo, useRef, useEffect, memo, useCallback } from "react";
...
const ObservationTreeTraceNode = memo(
  (props: {
    trace: Omit<TraceDomain, "input" | "output" | "metadata"> & {
      input: string | null;
      output: string | null;
    ...
    // Destructure to fix useCallback dependency
...
ObservationTreeTraceNode.displayName = "ObservationTreeTraceNode";
...
const ObservationTreeNodeCard = memo(
  ({
    observation,
    collapsed,
    ...
    const unnestedObservations = useMemo(
      () => unnestObservation(observation),
      [observation],
    );

    const totalCost = useMemo(
      () =>
        calculateDisplayTotalCost({ allObservations: unnestedObservations }),
      [unnestedObservations],
    );
    ...
    const handleToggleCollapse = useCallback(
      ...
      [toggleCollapsedObservation, observation.id, collapsed, capture],
    );
...
// Add display name for better debugging
ObservationTreeNodeCard.displayName = "ObservationTreeNodeCard";

// Export components for reuse in virtualized version
export { ObservationTreeTraceNode, ObservationTreeNodeCard };
